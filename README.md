...
| CO008 | 33 | 42 | ⚠️ 同上 |
| CO009 | 22 | 28 | ⚠️ hf/边数与手册偏差 |
| CO006 | 2 | 12 | ⚠️ DXF 仅 2 个 WM |
| CO010 | 99 | 166 | ⚠️ 统计口径差异（见下方分析）|
| **合计** | **315** | — | |
...
| 精确匹配 | 94 | **134** | +40 |
| 长度偏差 | 34 | **30** | -4 |
| Script-only | 35 | **22** | -13 |
| Manual-only | 65 | **38** | -27 |
| 总行数 | 282 | **315** | +33 |
...
- **非 BOM 几何推断**（`:644-678`）：对缺 BOM 的零件（p183/p197/p199 等）从 Part 几何推算 w/L/t 并注入 part_dims，使 CO 回退和 hf 修正覆盖全构件
...
| CO008 | 同类 + p101/p102 标签 | 同上 + 透传映射偏差 | ✗ |
| CO009 | hf/长度 ~6行 | 手册与几何不完全对应 | ⚠ |
...
**结论**：CO010 的 99/166 不是代码缺陷。约 55 行差额涉及工厂预制阶段的焊缝，需 CO010 的**子装配/零件图**对应的 DXF 文件才能覆盖。
