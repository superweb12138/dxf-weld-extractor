...
| 特性 | 说明 |
|------|------|
| **多阶段搜索** | Phase 1-2：先验式快速寻找无冲突位置；Phase 3：评分制枚举（30°×30档），选择最优 |
| **微调优化** | `_fine_tune` 在 Phase 3 最优位置附近 ±30° × 各距离档搜索更干净的落点 |
| **动态距离惩罚** | 无冲突时强推近距离（距-30）×4；有冲突时允许走远找干净位置 |
| **零件填充区检测** | 0-margin hatch bbox 精确判断文字是否在构件截面内，重叠扣 -5000 分 |
| **射线法优化** | 用 y/x 范围预过滤替代全量 17K 线遍历，效率提升 400× |
| **边界保护** | 超出视图边界时逐步缩短距离（8→原距离），找到不超界且不重叠 hatch 的最短距离 |
| **视图空间过滤** | `_collect_all_obstacles` 接收视图 bbox，只收集视图附近的模型空间实体 |
| **障碍物收集** | LINE/TEXT/MTEXT/CIRCLE/ARC/DIMENSION/ATTDEF/ATTRIB/MLEADER/LWPOLYLINE/SOLID/3DFACE |
...
所有 11 构件（BE018–BE023、CO006–CO010）均参与 Excel 统计输出。CO007/CO008/CO009 的 DXF 标注已完成优化（箭头基准定位、剖视图识别、x2_instances 镜像、多视图 TYP 分发），达到生产可用状态：

- **零标签重叠**：所有已标注视图的文字之间无重叠
- **引线不交叉文字**：冲突解决后处理后，引线不与已放置文字交叉
- **动态位置自适应**：拥挤视图自动转向评分制，无冲突视图快速定位
...
| 问题 | 影响 | 状态 |
|------|------|------|
| 多视图标注跨视图重叠 | 不同视图的标注在图纸上位置重叠时无全局冲突检测 | 待解决 |
| CO010 标注位置优化 | 部分视图（如 S-S、T-T）的 CJP/FW 标注位置通过 centroid 退化确定，不够精确 | 待优化 |

### 焊缝提取

| 问题 | 影响 | 状态 |
|------|------|------|
| 细部视图钢板对未完全覆盖 | 部分视图只有单块钢板（无 comp 截面），缺乏提取触发的几何前提 | 需按视图映射配置补齐 |
| CJP paired fillet 未覆盖非配置板 | `cjp_extra_fillet` 外板的配对填角焊不会自动生成 | 待扩展 |
