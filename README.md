...
- **D-D 短边填角**：水平边较短（宽 < 12）时 tip 取边中点，引线离开焊缝本体（避免贴柱角）
- **E-E 加劲/翼缘**：tip 落在实线翼缘边（y≈115），不落虚线延伸；下翼缘中点优先朝下进 H 口袋
...
| **梯子分槽** | 密区按焊点 Y 竖向分槽（一格一标）；共用走廊时左右视图 X 微交错 |
| **引线扇出** | 同簇近距平行引线强制上下分带（`_force_diverge_parallel_leaders`）；走廊内用 soft-clear |
| **右半 W 堆** | 右半区 `W*` 按 tip Y 高→低重排文字；高 tip 必须朝右上（约 45°–60°），避免 W1/W4/W5 交叉 |
| **引线长度** | 斜段硬上限 `MAX_DIAG_LEN = 50`，优先短引线 |
| **走廊空白** | C-C / D-D 等邻视图缝隙可作短距偏好；走廊 soft-clear 忽略膨胀 hatch pad，但仍禁真实线/WM 命中 |
| **碰撞** | 硬门控 `_label_hard_clear` + 走廊 soft；全局冲突解决 + 近距字分槽；蓝×蓝交叉尽量消除 |
...
| `LABEL_HEIGHT` | ≈剖面标题 | 由 `_detect_section_title_height` 对齐 A-A 字高 |
| `LEADER_CROSS_MIN_DEG` | 70 | 蓝×蓝交叉：优先消除；夹角≤70°非法（原 45°） |
...
DXF 文件
  ...
  └─ dxf_annotator.annotate → annotated/gb/*.dxf
       ...
       ├─ 象限归属 + 同半区分向
       ├─ 短距优先搜索 + 走廊偏好（不并入额外象限）
       ├─ 平行引线上下分带（走廊 soft-clear）/ 右半 W 按 tip Y 重排
       ├─ 冲突解决 / 蓝×蓝交叉修复 / 近距字分槽 / hard-clear 兜底
       └─ 绘制前角度钳位（水平/垂直 ±10°）
```
...
- 字体重叠：目标为 0
- 焊缝标注字号：与剖面标题（A-A / 1:10）一致，不再按密集区缩小
- 斜引线倾角：相对水平约 10°–80°
- **CO007/CO008**：D-D 走廊 fillet（如 F25）进一步左移避开绿 WM；E-E 右半 `W*` 按 tip 高度梯子排列，高 tip（如 W5/W6）朝右上
- 全流水线墙钟：约 5 分钟（BE018–CO009 标注；跳过 CO010）
- 验收以 **DXF** 为准；`annotated/**/*.PDF` 可能滞后于同次 DXF
...
| 问题 | 状态 |
|------|------|
| 极密区仍可能有少量引线交叉 | 已大幅改善；平行分带 + tip-Y W 堆已覆盖 CO007/CO008 主问题 |
| 走廊 soft-clear 与 hard gate 先后次序敏感 | 已固定 diverge→unpark→soft 扫；改序需回归 CO007/CO008 |
